        ...
        <div className="flex flex-col min-w-0">
          {currentView === 'AI' ? (
            ...
            <span className="text-[#1a1a1a] text-[20px] font-bold tracking-tight truncate">
              {title || (currentView === 'PEOPLE_LIST' ? 'Conversas' : currentView === 'WORKFLOW_DASHBOARD' ? 'Fluxos de Trabalho' : 'Chat')}
            </span>
          )}
        </div>
      </div>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white border border-[#e5e7eb] rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-1.5 animate-in fade-in zoom-in duration-200 origin-top-right">
              <button 
                onClick={() => {
                  onViewChange('WORKFLOW_DASHBOARD');
                  setIsMenuOpen(false);
                }}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors text-left ${currentView === 'WORKFLOW_DASHBOARD' ? 'bg-[#f7f6f2] text-[#1a1a1a]' : 'text-[#374151] hover:bg-slate-50'}`}
              >
                <span className="material-symbols-outlined text-[20px]">assignment</span>
                <span className="text-[15px] font-medium flex-1">Fluxo de trabalho</span>
              </button>
              ...
              <button className="w-full flex items-center gap-3 px-3 py-2.5 text-[#374151] hover:bg-slate-50 rounded-xl transition-colors text-left">
                ...
                <span className="text-[15px] flex-1">Configurações</span>
              </button>
            </div>
          )}
        </div>
